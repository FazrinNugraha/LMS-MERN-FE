  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-5 left-5 z-50 w-10 h-10 rounded-md bg-canvas border border-hairline flex items-center justify-center"
      ...
      {/* Sidebar */}
      <aside className={`sidebar-container fixed h-[calc(100vh-20px)] w-full max-w-[280px] my-[10px] ml-[10px] bg-surface-soft overflow-hidden flex flex-1 rounded-xl z-40 transition-transform duration-300 lg:translate-x-0 border border-hairline ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="scroll-container flex w-full overflow-y-scroll hide-scrollbar">
        <nav className="flex flex-col w-full h-fit p-[20px] md:p-[30px] gap-10 z-10">
          ...
          <ul className="flex flex-col gap-3 md:gap-4">
            <p className="font-semibold text-xs leading-[18px] text-muted uppercase tracking-[1.5px]">GENERAL
            </p>
            <li>
              <Link to="/manager" onClick={closeSidebar}>
                <div className="flex items-center gap-3 w-full rounded-md border p-[12px_16px] md:p-[14px_20px] transition-colors duration-300 hover:bg-primary hover:border-primary bg-canvas border-hairline group">
                  <img
                    ...
                    alt="icon"
                  />
                  <span className="font-semibold text-body group-hover:text-on-dark text-sm md:text-base transition-colors">
                    Overview
                  </span>
                </div>
              </Link>
