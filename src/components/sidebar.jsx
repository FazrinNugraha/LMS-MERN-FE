...
export default function Sidebar({ isAdmin = true }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`sidebar-container fixed h-[calc(100vh-20px)] w-full max-w-[280px] my-[10px] ml-[10px] bg-white overflow-hidden flex flex-1 rounded-[20px] z-40 transition-transform duration-300 lg:translate-x-0 shadow-xl border border-gray-200 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <ul className="flex flex-col gap-3 md:gap-4">
            ...
            <li>
              <Link to="/manager" onClick={closeSidebar}>
                <div className="flex items-center gap-3 w-full rounded-full border p-[12px_16px] md:p-[14px_20px] transition-all duration-300 hover:bg-[#662FFF] hover:border-[#662FFF] hover:shadow-md bg-gray-50 border-gray-200 group">
                  <img
                    src="/assets/images/icons/3dcube-white.svg"
                    className="w-5 h-5 md:w-6 md:h-6 flex-shrink-0 opacity-70 group-hover:opacity-100"
                    alt="icon"
                  />
